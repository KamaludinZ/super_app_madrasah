...
const AdminTugasPage = () => {
  ...
  const loadTugas = async () => {
    try {
      ...
      console.log('Admin Tugas loaded:', res.data);
  ...
  const handleDeleteConfirm = async () => {
    try {
      await api.delete(`/kelas/tugas/${selectedTugas.id}`);
      setDeleteOpen(false);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Kelola semua tugas pembelajaran</p>
      </div>
