...
export default function AdminVervalGTKPage() {
  ...
  const refresh = async () => {
    setLoading(true);
    try {
      // Fetch GTK requests (guru + tenaga_kependidikan) with optional status filter
      const params = {};
  ...
  const handleReject = async () => {
    ...
    if (!(await confirmDialog('Tolak perubahan data ini? User akan menerima catatan penolakan.'))) return;
    setActionLoading(true);
    try {
  ...
  const userTypeBadge = (userType) => {
    if (userType === 'guru') return <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200">Guru</Badge>;
    if (userType === 'tenaga_kependidikan') return <Badge variant="outline" className="bg-purple-50 text-purple-700 border-purple-200">Tendik</Badge>;
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          <Badge className="bg-[#006837]/10 text-[#006837] border-[#006837]/20 mb-2">
            <Users className="h-3 w-3 mr-1" /> Verval Data GTK
          </Badge>
          <h1 className="text-2xl sm:text-3xl font-bold">Verifikasi & Validasi Data GTK</h1>
              ...
              <TableBody>
                {loading ? (
                  ...
                  <TableRow key={req.id}>
                    ...
                    <TableCell>{userTypeBadge(req.user_type)}</TableCell>
              ...
              <>
                <div className="bg-slate-50 border rounded-lg p-3 text-sm">
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div><span className="text-slate-600">Jenis:</span> {userTypeBadge(selectedRequest.user_type)}</div>
                    ...
                    {selectedRequest.reviewed_by_name && (
                      <div className="col-span-2"><span className="text-slate-600">Direview oleh:</span> {selectedRequest.reviewed_by_name}</div>
                    )}
                  </div>
                </div>
