...
const WakaKurMateriPage = () => {
  ...
  const loadMateri = async () => {
    try {
      setLoading(true);
      setError('');
      const res = await api.get('/wakakur/materi');
      console.log('Waka Kur Materi loaded:', res.data);
  ...
  const handleViewDetail = async (materi) => {
    try {
      const res = await api.get(`/wakakur/materi/${materi.id}`);
    ...
    <div className="space-y-6">
      <div>
        <Badge className="bg-[#006837]/10 text-[#006837] border-[#006837]/20 mb-2">
          <BookOpen className="h-3 w-3 mr-1" /> Materi Mapel (Waka Kurikulum)
        </Badge>
        <h1 className="text-3xl font-bold">Materi Mapel</h1>
        <p className="text-muted-foreground">Lihat semua materi pembelajaran (Read-Only)</p>
      </div>
...
export default WakaKurMateriPage;
