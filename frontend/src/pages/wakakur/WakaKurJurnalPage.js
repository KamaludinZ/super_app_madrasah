...
import { ClipboardList, Filter, RefreshCw, BarChart3, Loader2, Eye, FileSpreadsheet } from 'lucide-react';
  ...
  const load = async () => {
    ...
    try {
      const [r, s] = await Promise.all([
        api.get('/wakakur/jurnal', { params: buildParams() }),
        api.get('/wakakur/jurnal/stats-by-teacher'),
      ]);
    ...
    <div className="space-y-6">
      <div>
        <Badge className="bg-[#006837]/10 text-[#006837] border-[#006837]/20 mb-2">
          <ClipboardList className="h-3 w-3 mr-1" /> Data Jurnal (Waka Kurikulum)
        </Badge>
        <h1 className="text-2xl sm:text-3xl font-bold text-slate-900">Rekap Jurnal Mengajar</h1>
        <p className="text-sm text-slate-600 mt-1">{data.total} entri jurnal {filters.class_id !== 'all' || filters.teacher_id !== 'all' || filters.subject_id !== 'all' || filters.start_date || filters.end_date ? '(terfilter)' : ''} (Read-Only)</p>
      </div>
      ...
      <Tabs defaultValue="items">
        ...
        <TabsContent value="items" className="mt-4">
          {loading ? <Loader2 className="h-5 w-5 animate-spin mx-auto" /> : (
            <Card><CardContent className="p-0"><div className="overflow-x-auto"><Table data-testid="wakakur-jurnal-rekap-table">
