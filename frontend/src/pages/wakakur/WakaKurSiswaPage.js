...
import {
  Users, Search, GraduationCap, Eye, KeyRound, Loader2,
  UserCheck, UserX,
} from 'lucide-react';
...
export default function WakaKurSiswaPage() {
  const { activeRole, user } = useAuth();
  const canViewAccount = true; // Waka Kurikulum can view account info
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // Load classes for active academic year using waka kurikulum endpoint
        const c = await api.get('/wakakur/classes', {
  ...
  const loadStudents = async (classId) => {
    ...
    try {
      ...
      const { data } = await api.get('/wakakur/siswa', { params });
      setStudents(data || []);
    } catch (e) { /* ignore */ }
    finally { setLoading(false); }
  };
  ...
  return (
    <div className="space-y-6" data-testid="wakakur-siswa-page">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          <Badge className="bg-[#006837]/10 text-[#006837] border-[#006837]/20 mb-2">
            <Users className="h-3 w-3 mr-1" /> Data Siswa (Waka Kurikulum)
          </Badge>
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-900">Daftar Siswa</h1>
          <p className="text-sm text-slate-600 mt-1">
            Semua siswa madrasah (Read-Only)
          </p>
        </div>
      </div>
