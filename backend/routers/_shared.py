...
async def user_can_view_class(user: Dict, class_id: str) -> bool:
    ...
    if cls and cls.get('homeroom_teacher_id') == user['id']:
        return True
    # Allow guru_bk, guru_tata_tertib, guru_piket, tendik, UKS staff, and kepala sekolah to view all
    overlap = set(user.get('roles', [])) & {'guru_bk', 'guru_tata_tertib', 'guru_piket', 'tenaga_kependidikan', 'unit_kesehatan', 'kepala_sekolah'}
    if overlap:
        return True
