"""
Complete Data Seeder for Local MongoDB Docker
Adapted from seed_all_data.py for local development
"""
import asyncio
import uuid
import os
...
async def seed_all_data(db: Any):
    ...
    print("\n" + "="*60)
    print("SEEDING ALL APPLICATION DATA - LOCAL MONGODB")
    print("="*60 + "\n")
    ...
    # ============================================================
    # 10-15: Rest of the data (Achievements, Announcements, etc.)
    # ============================================================
    ...
    print("  - Guru: username=guru1, password=guru123")
    print("  - Siswa: username=siswa1, password=siswa123")
    print("="*60 + "\n")
...
async def main():
    ...
    MONGO_ROOT_PASSWORD = os.getenv('MONGO_ROOT_PASSWORD', 'SuperStrongPassword2024!SecureMongo')
    DB_NAME = os.getenv('DB_NAME', 'super_app_madrasah')
    MONGO_URL = os.getenv('MONGO_URL', f'mongodb://{MONGO_ROOT_USERNAME}:{MONGO_ROOT_PASSWORD}@localhost:27017/{DB_NAME}?authSource=admin')

    print(f"\nConnecting to Local MongoDB Docker...")
    print(f"URL: {MONGO_URL.replace(MONGO_ROOT_PASSWORD, '***')}")
